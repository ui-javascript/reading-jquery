...
duma.BeautifyScrollBar = function (obj, arrowUpCss, arrowUpActiveCss, handleCss, handleActiveCss, arrowDownCss, arrowDownActiveCss) {
  ...
  var innerChildren = $(this.obj).children().detach();
  //var innerChildren = $(this.obj).children();
  /* 20150820 兼容ie7ie8*/
  ...
  $(this.obj).html('<div id="' + obj + 'Area" class="dumascroll_area"></div><div id="' + obj + 'Bar" class="dumascroll_bar"><div class="dumascroll_arrow_up"></div><div class="dumascroll_handle"></div><div class="dumascroll_arrow_down"></div></div>');
  this.area = duma.$(obj + "Area");
  $(this.area).append(innerChildren);
  ...
  this.handleHeight = parseInt(this.area.offsetHeight / this.area.scrollHeight * (this.bar.offsetHeight - this.arrowUpHeight - this.arrowDownHeight) - 60);
}
duma.BeautifyScrollBar.prototype = {
  ...
  sethandle: function () { //当内容超多时设置拖拽条子的最小高度
    ...
    if (this.handleHeight > this.handleMinHeight) {
      this.handleHeight = this.handleMinHeight;
      this.handle.style.height = this.handleMinHeight + "px";
      // 不理解下面这段直接注释
  ...
  clearBarMouseDownInterval: function () {
    clearInterval(this.barMouseDownInterval);
  },
  areaScroll: function () {
    if (this.area.scrollHeight)
    ...
    if (this.handle.style.top === '281px') {
      var data = {}
      data['list'] = 0
      $wq.retrieveGold.setQueryResult(data)
    }
  },
  ...
  barScroll: function (e) {
    var e = e || window.event,
      eDir; //设置滚轮事件,e.wheelDelta与e.detail分别兼容IE、W3C，根据返回值的正负来判断滚动方向
    if (e.wheelDelta) {
    ...
    if (this.area.scrollTop > 0 && this.area.scrollTop < this.area.scrollHeight - this.area.offsetHeight) {
      duma.stopEvent(e);
    }
    this.setBarPos();
  },
  barDown: function (e) {
    ...
    if (documentScrollTop + eY < hOffset.top) {
      this.barMouseDownInterval = setInterval(function (e) {
        ...
        if (that.area.scrollTop <= (eY + documentScrollTop - bOffset.top - that.arrowUpHeight) / (that.bar.offsetHeight - that.arrowUpHeight - that.arrowDownHeight) * that.area.scrollHeight) {
          that.clearBarMouseDownInterval();
        }
        that.setBarPos();
      }, 80);
    } else if (documentScrollTop + eY > hOffset.top + hOffset.height) {
      this.barMouseDownInterval = setInterval(function () {
        that.area.scrollTop += that.area.offsetHeight;
  ...
  arrowUpMouseDown: function (e) {
    var that = this;
    this.arrowUpInterval = setInterval(function () {
      that.area.scrollTop -= 10;
      that.setBarPos();
    }, 30);
  ...
  arrowUpMouseUp: function () {
    ...
    this.arrowUp.className = this.arrowUpCss;
  },
  ...
  arrowDownMouseDown: function (e) {
    var that = this;
    this.arrowDownInterval = setInterval(function () {
      that.area.scrollTop += 10;
      that.setBarPos();
    }, 30);
  ...
  arrowDownMouseUp: function () {
    ...
    this.arrowDown.className = this.arrowDownCss;
  },
  ...
  run: function () {
    ...
    this.area.onmouseover = this.bar.onmouseover = function (event) {
      that.areakeydown(event)
    };
    this.area.onmouseout = this.bar.onmouseout = function () {
...
function fixHeight(options) {
  var bodyHeight = $(options).height();
  $('#user_frame', window.parent.document).height(bodyHeight)
}
...
(function ($) {
  ...
  $.fn.unbindInputChange = function (fn) {
    if (/msie/i.test(navigator.userAgent)) {
      ...
      //非ie浏览器，比如Firefox
      $(this).get(0).addEventListener("input", function () {}, false);
    }
  };
...
var employeeView = {
  ...
  bindView: function (options) {
    ...
    var target = options.target;
    var trigger = $('<div class="view_btn clearfix" style="visibility:hidden;overflow:hidden; width: 0px;height: 0px;" href="javascript:void(0)">视图</div>');
    target.parent().append(trigger);
    ...
    trigger.weqiafloat({
      ...
      //20151014  //增加了 在显示前的调用函数   修改了weqia_float.js
      beforeShowCall: function () {
        employeeView.unemployeeInfo(options);
        employeeView.employeeInfo(options);
      }
    });
  ...
  createView: function () {
    if (!this.view || !this.view.length) {
      var view = this.view = $('<div class="task_y_main task_view_box" style="display:none;"></div>');
      var view_c = $('<div class="task_y_conter"></div>');
      view.append(view_c);
      view_c.append('<div class="task_y_xian"><p class="customer_name"></p></div>');
      view_c.append('<div class="task_y_xian"><p class="t_customer_trade"></p></div>');
      view_c.append('<div class="task_y_xian"><p class="t_customer_level"></p></div>');
      view_c.append('<div class="task_y_xian"><p class="t_customer_class"></p></div>');
      view_c.append('<div class="task_y_xian"><p class="t_customer_addName"></p></div>');
      $(document.body).append(view);
    }
    return this.view;
  },

  //20151014 卸载任务信息   现阶段   先卸载描述
  unemployeeInfo: function (options) {
    var target = options.target;
    var employeeInfo = target.data("info");
    if (!employeeInfo) {

      var employeeInfo = employeeView.schemployeeInfo(options.mid);
      target.data("info", employeeInfo.employeeInfo);
    }
  },

  employeeInfo: function (options) {
    var target = options.target;
    var empnolyInfo = target.data("info");
    if (empnolyInfo) {
      var mName = empnolyInfo.mName;
      var sex = empnolyInfo.sex || '';
      if (sex == 1) {
        sex = '男';
      } else {
        sex = '女';
      }
      var departmentName = empnolyInfo.departmentName || '';
      var linkPhone = empnolyInfo.mobile || '';
      var email = empnolyInfo.email || '';
      var view = this.createView();
      view.find(".customer_name").html('<div>姓名：<span> ' + mName + '</span></div>');
      view.find(".t_customer_trade").html('<div>性别：<span> ' + sex + '</span></div>');
      view.find(".t_customer_level").html('<div>部门：<span> ' + departmentName + '</span></div>');
      view.find(".t_customer_class").html('<div>联系电话：<span> ' + linkPhone + '</span></div>');
      view.find(".t_customer_addName").html('<div>邮箱：<span> ' + email + '</span></div>');
    }
  },

  schemployeeInfo: function (mid) {

    var url = "/front/contact/employeeInfo.htm";
    return $wq.getJsonData({
      url: url,
      data: {
        "query.member_id": mid
      }
